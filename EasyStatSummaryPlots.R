...
ggEasy.scatter.stat<-function(df, xval = 1, yval = 2, color = 3, errortype = "sd", connectLine = FALSE, 
  ...
  g <- ggplot(df, aes(x = df[,xval], y = df[,yval], color = df[,color]))
  if (errortype == "cl") {
  g <- g + stat_summary(fun.data = mean_cl_boot, alpha = alpha, size = point.size)
    ...
    if (errortype == "sd") {
      g <- g + stat_summary(fun.data = mean_sdl, geom = "errorbar", width = error.width, size = error.width * 3, alpha = alpha)
    } else if (errortype == "sem") {
      g <- g + stat_summary(fun.data = mean_se, geom = "errorbar", width = error.width, size = error.width * 3, alpha = alpha)
    }
  }
...
# sd = standard deviation
# sem = standard error of the mean
ggEasy.barplot.stat<-function(df, xval = 1, yval = 2, color = 3, shape = NULL, errortype = "sd", allpoints = T,
                              error.width = 0.15, point.size = 1, alpha = 0.8, width = 8, height = 6,
                              wespalette = "Darjeeling1") {
  g <- ggplot(df,aes(x = factor(df[,xval]), y = df[,yval], color = factor(df[,color]), fill = factor(df[,color]))) + 
