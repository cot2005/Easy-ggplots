ggEasy.CorPlot<-function(df, xval = 1, yval = 2, fill = 3, label = 4, facetGroup = NULL, cortest = "pearson", point.size = 2.5, width = 8, height = 8, outputfile = "facet_corplot.pdf") {
  cols <- colnames(df)
  #puts correlation result in upper left quadrant
  xlabcoord <- (max(df[,xval]) - min(df[,xval])) * 0.1 + min(df[,xval]).  # used to place the corr annotation
